...
function SubmitButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="w-full btn-primary disabled:opacity-50 disabled:cursor-not-allowed font-semibold text-base sm:text-sm py-3.5 sm:py-3 rounded-xl shadow-lg transition-all min-h-[48px] hover:opacity-90"
    >
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted text-sm mt-0.5">Fill in the class you just completed.</p>
      </div>
      ...
      <form ref={formRef} key={formKey} action={formAction} className="space-y-5">
        ...
        {state.success && (
          <div className="rounded-xl bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800/40 px-4 py-3">
            <p className="text-sm text-emerald-800 dark:text-emerald-200 font-medium">Session saved — you can log another below.</p>
          </div>
        )}
        ...
        <div className="rounded-xl border border-default bg-muted/50 p-4 space-y-4">
          ...
          <div>
            <p className="text-[11px] text-subtle mb-2">Quick length (sets End from Start)</p>
            <div className="flex flex-wrap gap-2">
              {DURATION_PRESETS.map((p) => (
                <button
                  ...
                  className="text-sm sm:text-xs font-medium px-4 py-2.5 sm:px-3 sm:py-1.5 rounded-lg border border-default text-primary hover:border-[var(--accent)] hover:text-accent disabled:opacity-40 disabled:cursor-not-allowed transition-colors min-h-[44px]"
                >
                  {p.label}
                </button>
              ))}
            ...
            className={`rounded-lg px-3 py-2.5 text-sm ${
              durationPreview.valid
                ...
                : startComplete || endComplete
                  ...
                  : 'bg-muted/50 border border-default text-subtle'
            }`}
          >
            {durationPreview.valid ? (
              <span>
